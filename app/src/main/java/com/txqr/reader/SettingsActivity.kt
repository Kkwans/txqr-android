...
class SettingsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val switchAlwaysShow = findViewById<Switch>(R.id.switchAlwaysShowProgress)
        ...
        switchAlwaysShow.isChecked = prefs.getBoolean(KEY_ALWAYS_SHOW_PROGRESS, false)
    ...
    private fun showHelp(title: String, message: String) {
        AlertDialog.Builder(this, R.style.RoundedDialog).setTitle(title).setMessage(message).setPositiveButton("知道了", null).show()
    }
    ...
    private fun showResolutionPicker() {
        ...
        AlertDialog.Builder(this, R.style.RoundedDialog).setTitle("分析分辨率")
            .setSingleChoiceItems(RESOLUTION_LABELS, idx) { d, w -> prefs.edit().putString(KEY_RESOLUTION, RESOLUTION_VALUES[w]).apply(); updateResolutionDisplay(); d.dismiss() }
    ...
    private fun updateDirDisplay() {
        ...
        tvSaveDir.text = when { p.isNotEmpty() -> p.removePrefix("/storage/emulated/0").let { if (it.startsWith("/")) it else "/$it" }; u.isNotEmpty() -> "已选择目录"; else -> "/Download/TXQR" }
    }

    private fun chooseDirectory() {
        try { startActivityForResult(Intent(Intent.ACTION_OPEN_DOCUMENT_TREE).apply { addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION or Intent.FLAG_GRANT_WRITE_URI_PERMISSION or Intent.FLAG_GRANT_PERSISTABLE_URI_PERMISSION) }, REQUEST_CODE_OPEN_DOCUMENT_TREE) }
